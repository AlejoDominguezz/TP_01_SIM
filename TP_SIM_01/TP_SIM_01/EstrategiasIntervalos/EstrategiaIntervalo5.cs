...
{
    class EstrategiaIntervalo5 : IEstrategiaIntervalos
    {

        public override List<int> frecuenciaObservada(List<float> valoresAleatorios)
        {
            List<int> fo = new List<int> { 0, 0, 0, 0, 0 };
            for (int i = 0; i < valoresAleatorios.Count; i++)
            {
                int posicion = (int)Math.Truncate((float)valoresAleatorios[i] / (float)0.20);
        ...
        {
            float[,] intervalos = new float[5, 2];
            for (int i = 0; i < 5; i++)
            {
                intervalos[i, 0] = (float)0.20 * i;
                intervalos[i, 1] = (float)(0.20 * i + 0.1999);
            }
            return intervalos;
        }
